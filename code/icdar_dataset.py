...
# def translate_image(image, vertices, x_offset, y_offset):
#     """
#     Translate the image and vertices by x_offset and y_offset.
#     """
...
def adjust_brightness_contrast_saturation(image):
    """
    Adjust brightness, contrast, and saturation of the image with random factors.
    Brightness, contrast, and saturation are adjusted within predefined random ranges.
    """
    # 랜덤 범위 설정
    brightness_factor = np.random.uniform(0.8, 1.2)  # 밝기 조절 (0.8 ~ 1.2)
    contrast_factor = np.random.uniform(0.8, 1.5)    # 대비 조절 (0.8 ~ 1.5)
    saturation_factor = np.random.uniform(0.8, 1.5)  # 채도 조절 (0.8 ~ 1.5)
...
class ICDARSceneTextDataset(Dataset):
    def __init__(self, root_dir,
        ...
        with open(osp.join(root_dir, 'ufo/{}.json'.format(split)), 'r', encoding='utf-8') as f:
            anno = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        if self.apply_augments:
            ...
            #     image = np.array(image)
             ...
             if np.random.rand() > 0.5:
                 image = adjust_brightness_contrast_saturation(Image.fromarray(image))
                 image = np.array(image)
            ...
            # if np.random.rand() > 0.5:
            #     image = overlay_text(Image.fromarray(image))
            #     image = np.array(image)
